...
public abstract class RBMDatabook implements DataReader, DataWriter {
	...
	private static File validateAndResolveInputFile( File inputFile ) throws IOException {
		...
		return validateAndResolveInputFile( inputFile, new File(tmpDir) );
	}

	public static File validateAndResolveInputFile( File inputFile, File allowedBaseDir ) throws IOException {
		if( inputFile == null || allowedBaseDir == null )
			throw new IOException( DataException.ERR_IO_INVALID_REQUEST_FILETYPE );

		String rawPath = inputFile.getPath();
		if( rawPath == null || rawPath.length() == 0 || rawPath.indexOf("..") >= 0 )
			throw new IOException( DataException.ERR_IO_INVALID_REQUEST_FILETYPE );

		Path basePath = allowedBaseDir.toPath().toAbsolutePath().normalize();
		Path candidatePath = inputFile.toPath().toAbsolutePath().normalize();
		if( !candidatePath.isAbsolute() || candidatePath.getParent() == null )
			throw new IOException( DataException.ERR_IO_INVALID_REQUEST_FILETYPE );

		// Pre-check containment before resolving real path.
		if( candidatePath.equals(basePath) || !candidatePath.startsWith(basePath.resolve("")) )
			throw new IOException( DataException.ERR_IO_INVALID_REQUEST_FILETYPE );

		Path filePath = candidatePath.toRealPath();
		Path resolvedBasePath = basePath.toRealPath();

		// Re-check containment after resolving real path to prevent symlink bypass.
		if( filePath.equals(resolvedBasePath) || !filePath.startsWith(resolvedBasePath.resolve("")) )
			throw new IOException( DataException.ERR_IO_INVALID_REQUEST_FILETYPE );

		File resolvedFile = filePath.toFile();
